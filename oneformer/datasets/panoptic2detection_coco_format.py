...
def convert_panoptic_to_detection_coco_format_single_core(
    ...
    for working_idx, annotation in enumerate(annotations_set):
        ...
        try:
            pan_format = np.array(
                Image.open(os.path.join(segmentations_folder, file_name)),
                dtype=np.uint32,
            )
        except IOError:
            raise KeyError(
                "no prediction png file for id: {}".format(annotation["image_id"])
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--things_only", action="store_true", help="discard stuff classes"
    )
    ...
    output_json_file = os.path.join(
        root, "annotations", "panoptic2instances_val2017.json"
    )
